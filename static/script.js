...
document.addEventListener('DOMContentLoaded', function () {
    ...
    script.onload = function () {
        ...
        window.startGame = function () {
            ...
            document.getElementById('btn-leavewriting').style.display = 'none'
        };
        ...
        window.leaveWaitingRoom = function () {
            ...
            if (username && room) {
                socket.emit('leave_waiting_room', { username, room });
        ...
        socket.on('user_left_waiting' , data => {
            const playerElement = document.getElementById(data.username);
            if (playerElement) {
                playerElement.remove();
            }
            const confirmationBox = document.getElementById('confirmationBox');
            const messageElement = document.createElement('p');
            messageElement.textContent = `Player ${data.username} has left the waiting room.`
            confirmationBox.appendChild(messageElement);
        })
